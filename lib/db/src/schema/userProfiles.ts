...
export const userProfilesTable = pgTable(
  "user_profiles",
  {
    ...
    // Uniqueness is case-insensitive — enforced via the expression unique index below.
  ...
  (t) => [
    uniqueIndex("user_profiles_display_name_lower_uq").on(sql`lower(${t.displayName})`),
  ],
);
